...
- [x] **【账号管理】** 简单的权限配置
    ...
    - [ ] 流量限制
- [x] **【tcp转发】** 转发tcp协议
    - tcp <----> A客户端 <--打洞/服务器中继/节点中继--> B客户端<----> 内网tcp
- [x] **【udp转发】** 转发udp协议
    - udp <----> A客户端 <--打洞/服务器中继/节点中继--> B客户端<----> 内网udp
- [x] **【http代理】**  以节点或者服务端作为代理目标
    - http代理 <----> A客户端 <--打洞/服务器中继/节点中继--> B客户端 <----> 外网
    - http代理 <----> A客户端 <----> 服务器 <----> 外网
- [x] **【socks5代理】** 以节点或者服务端作为代理目标
    - socks5 <----> A客户端 <--打洞/服务器中继/节点中继--> B客户端 <----> 外网
    - socks5 <----> A客户端 <----> 服务器 <----> 外网
- [x] **【虚拟网卡组网】** 
    - 虚拟ip(tcp/udp) <----> A客户端 <--打洞/服务器中继/节点中继--> B客户端<----> 内网(tcp/udp)
