    ...
    public partial class NetManager
    {
        ...
        private void ReceiveLogic(object state)
        {
            ...
            if (socket.Available == 0 && !socket.Poll(ReceivePollingTime, SelectMode.SelectRead))
                return;
            NetPacket packet = PoolGetPacket(NetConstants.MaxPacketSize);
            socket.BeginReceiveFrom(packet.RawData, 0, NetConstants.MaxPacketSize, SocketFlags.None, ref bufferEndPoint, ReceiveCallback, new ReceiveModel
            {
                Packet = packet,
                Socket = socket
            });
        ...
        {
            ReceiveModel model = result.AsyncState as ReceiveModel;

            try
            {
                EndPoint bufferEndPoint = new IPEndPoint(model.Socket.AddressFamily == AddressFamily.InterNetwork ? IPAddress.Any : IPAddress.IPv6Any, 0);
                model.Packet.Size = model.Socket.EndReceiveFrom(result, ref bufferEndPoint);
                OnMessageReceived(model.Packet, (IPEndPoint)bufferEndPoint);


                if (model.Socket.Available == 0 && model.Socket.Poll(ReceivePollingTime, SelectMode.SelectRead) == false)
                {
                    return;
                }
                if (IsActive() == false)
                ...
                NetPacket packet = PoolGetPacket(NetConstants.MaxPacketSize);
                model.Packet = PoolGetPacket(NetConstants.MaxPacketSize);
                model.Socket.BeginReceiveFrom(packet.RawData, 0, NetConstants.MaxPacketSize, SocketFlags.None, ref bufferEndPoint, ReceiveCallback, model);
            }
            catch (SocketException ex)
            {
                if (ProcessError(ex))
