...
dotnet publish ./server/server.service -c release -f net7.0 -o ./public/publish/linux-alpine-x64/server -r alpine-x64 --self-contained true  -p:DebugType=none -p:DebugSymbols=false  -p:PublishSingleFile=true -p:PublishTrimmed=true -p:IncludeNativeLibrariesForSelfExtract=true -p:EnableCompressionInSingleFile=true
